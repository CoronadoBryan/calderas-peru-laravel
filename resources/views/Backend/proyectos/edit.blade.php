<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8">
            <h2 class="text-3xl font-bold mb-8 text-gray-800 dark:text-gray-200">{{ __('Editar Proyecto') }}</h2>
            <p class="text-sm text-gray-500 dark:text-gray-400 mb-8">
                {{ __('Edita los campos del proyecto. Solo los campos que modifiques serán actualizados.') }}
            </p>

            <form method="POST" action="{{ route('proyectos.update', $proyecto->id) }}" enctype="multipart/form-data" class="space-y-8">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-2">
                    ...
                    <textarea name="descripcion" id="descripcion" rows="4"
                        class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:bg-gray-700 dark:border-gray-600 dark:text-gray-300">{{ old('descripcion', $proyecto->descripcion) }}</textarea>
                    @error('descripcion')
                        <span class="text-red-500 text-sm">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="space-y-4">
                    ...
                    <p class="text-sm text-gray-500 dark:text-gray-400 mb-4">
                        {{ __('Puedes reemplazar las imágenes existentes o dejarlas como están.') }}
                    </p>
                            ...
                            <div class="space-y-2">
                                <div class="relative group">
                                    ...
                                    <div id="preview_container_{{ $i }}" class="w-full h-48 bg-gray-100 dark:bg-gray-700 rounded-lg flex items-center justify-center overflow-hidden border-2 border-dashed border-gray-300 dark:border-gray-600 mb-2">
                                        @if (!empty($proyecto["imagen_$i"]))
                                            <img id="preview_{{ $i }}" class="w-full h-full object-cover" src="{{ asset('storage/' . $proyecto["imagen_$i"]) }}" alt="Imagen {{ $i }}">
                                        @else
                                    ...
                                    <!-- File Input -->
                                    <input type="file" name="imagen_{{ $i }}" id="imagen_{{ $i }}" @if ($i == 1) @endif accept="image/jpeg,image/png,image/jpg"
